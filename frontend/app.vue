			...
			<div class="flex justify-between w-full items-center mb-8">
				<form class="w-full sm:w-[80%] md:w-[60%] lg:w-[50%] mx-auto mt-10" @submit.prevent="fetchWeather">
					...
					<div class="relative">
						...
						<button @keydown.native.enter="fetchWeather" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-2">Search</button>
					</div>
				</form>
			</div>

			<div class="text-center rounded-xl w-full max-w-md mt-10" v-if="loading">
...
const fetchWeather = async (): void => {
	...
	const {
		...
		status,
		error: errorFetch,
	} = await useFetch(`http://localhost:2005/api/weather`, {
		method: 'GET',
...
const formatTemperature = (temp: string): string => temp.toFixed(1);
const getWeatherIcon = (icon: string): string => `https://openweathermap.org/img/wn/${icon}@2x.png`;
