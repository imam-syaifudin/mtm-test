<template>
	<div class="w-[100%]">
		<h1 class="text-center font-bold capitalize">Data prakiraan cuaca untuk beberapa hari ke depan</h1>
		<form class="w-full sm:w-[80%] md:w-[30%] lg:w-[20%] mt-10 ms-auto mb-10" @submit.prevent="fetchWeather">
			...
			<div class="relative">
				...
				<button @click="fetchWeather" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-2" v-loading="loading">Search</button>
			</div>
		</form>
		...
		<GridLayout v-model:layout="layout" :row-height="30" :col-num="12">
			<template #item="{ item }">
				<Line v-if="item.type === 'line'" :data="item.data" :options="chartOptions" />
...
const fetchWeather = async () => {
	...
	const {
		...
		status,
		error: errorFetch,
	} = await useFetch(`http://localhost:2005/api/weather/forecast`, {
		method: 'GET',
